...
| `0` | 音量を上げる |
| `9` | 音量を下げる |
| `e` | 音声の加工（なし→8bit→4bit）を切り替える |
| `s` | 再生中の動画を手元へ保存して登録する |

### 8bitサウンド

`--audio-effect 8bit` を付けると，音声をレトロゲーム風の粗い音にできます．文字で描かれた映像との相性が良い組み合わせです．

```bash
fraterm run video.mp4 --audio -e 8bit    # 8bit相当の粗さ
fraterm run video.mp4 --audio -e 4bit    # さらに粗く，ざらついた音
```

再生中に `e` を押すと「なし → 8bit → 4bit」の順で切り替わり，ステータス行に `音声:100%/8bit` と表示されます．

ビット深度を落とす `acrusher` と，標本化周波数を下げる `aresample` を組み合わせて実現しています（8bitは11025Hz，4bitは8000Hz）．再生速度の変更とも同時に使えます．
...
```bash
fraterm defaults -m ascii -s detailed -c true -b chrome   # 一度だけ設定する
...
| `--audio-effect none\|8bit\|4bit` | 音声を粗くしてレトロゲーム風にします |
