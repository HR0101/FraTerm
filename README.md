...
| `r` | 先頭から再生し直す |
| `←` / `→` または `h` / `l` | 10秒前後へ移動する |
| `0` 〜 `9` | 動画の0%〜90%へ移動する（長さを取得できない配信では，その旨を知らせて何もしません） |
| `m` | ミュートを切り替える（`--audio` 指定時のみ） |
| `+` | 再生速度を上げる |
| `-` | 再生速度を下げる |
| `[` / `]` | 音量を下げる / 上げる（`--audio` 指定時のみ） |
...
- 音声は `ffplay` の別プロセスで再生します．起動時は `ffplay` の進捗から実際の音声位置を取得して映像クロックを合わせ，取得できない場合だけ最大1.5秒待って再生を始めます．ずれを感じる場合は `--audio-offset` で微調整してください（正の値で音声が先行します）．
